...
**Phase 30: Form Hardening And Status Feedback** — Concluido. Novo helper `parseMoneyToCents`/`formatCentsToInput` em `src/lib/money.ts` (9 testes em `money.test.ts`) normaliza virgula/ponto, clampa negativos e arredonda sub-centavos. Componentes `MoneyInput` e `IntegerInput` em `src/components/ui/money-input.tsx` bloqueiam `e/+/-`, desarmam o scroll do mouse (blur on wheel) e escondem spinners via `appearance:textfield`. Os formularios de transacao, recorrencia, meta, pagamento de fatura e conta (incluindo dias de fechamento/vencimento como `IntegerInput`) passaram a usar o padrao; conversao para centavos agora vai por `parseMoneyToCents` em todos eles. A barra de filtros de `/transactions` virou container com `bg-card`, borda e sombra leve para ganhar contraste em fundos claros. Recorrencias pausadas em `recurring-list.tsx` ganharam fundo ambar suave (`bg-amber-50/60` + dark variant), descricao riscada em `muted` e badge com icone `Pause` no lugar do antigo `opacity-50` neutro.

## Next Planned Step

**Phase 31: Progressive Disclosure For Dense Lists** — evoluir a leitura de listas longas (transacoes, recorrencias, metas) conforme `.docs/tasks/` quando a task for criada, mantendo o foco em UX sem alterar contratos de API.
