...
**Phase 37 — Categories Page Redesign** — Concluida. O layout de `/categories` foi completamente reformulado com novo header (titulo + subtitulo + busca client-side + botao de criar), stats row com 4 cards (total de categorias, receitas, despesas, subcategorias), cards de secao com header colorido (teal para receitas, amber para despesas) com botao "+ Adicionar" pré-filtrando o formulario por tipo, expand/collapse de subcategorias por chevron (colapsadas por padrão), icone circular maior (32px para pais, 20px para filhos) com linha vertical de hierarquia, e busca que filtra pais e filhos mantendo o pai visível se qualquer filho bater na query. Novos componentes: `categories-content.tsx` (client-side wrapper com filtragem) e `category-stats.tsx` (4 cards de resumo). Componentes modificados: `page.tsx` (novo header), `category-list-card.tsx` (header/footer redesenhados), `category-list.tsx` (expand/collapse + icones maiores), `category-form.tsx` (prop `defaultType` adicionada). Schema e APIs intocados.

## Next Planned Step

**Formalizar a proxima phase do backlog de produto** — com prioridade natural para `Import/export CSV`, ja considerando lookup por `CreditCardPurchase`, parcelas futuras e parcelas adiantadas no export/import, ou avançar para um novo modulo como `PWA / responsivo mobile` focando em adaptacao de layouts para telas menores.

**Validacao visual/manual da nova UX de categorias** — rodar uma rodada manual focada em `/categories` para revisar collapse/expand, busca, icones, cores, dark mode e clareza do novo header com stats.
...
- **Post-Phase 36 UI polish**: `Button` ganhou variantes `choice`/`choice-active` com o azul `#38BDF8`; `DatePicker` reutilizavel com `Popover` + calendario customizado substitui inputs nativos de data em transacoes, compra da wishlist, novo item desejado e recorrencias; modais de transacao/compra/recorrencia e cards de wishlist agora usam o mesmo tom azul e efeitos de hover/press; o `Select` de conta de transacoes nao alterna mais entre controlado e uncontrolled
...
- **Categories page redesign (Phase 37)**: novo layout com header informativo (titulo + subtitulo + busca client-side + botao de criar), stats row com 4 cards (total, receitas, despesas, subcategorias), cards de secao teal/amber com botao "+ Adicionar" pré-filtrando por tipo, expand/collapse de subcategorias por chevron (padrão colapsado), icone maior (32px pais, 20px filhos) com linha vertical de hierarquia, e busca filtrando pais/filhos com pai visível se qualquer filho bate na query; novos arquivos `categories-content.tsx` e `category-stats.tsx`
