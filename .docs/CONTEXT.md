...
- **Transactions page redesign (Phase 39)**: novo layout com 4 cards de stats no topo (Receitas, Despesas, Saldo do período, Total de transações) calculados server-side via `groupBy`, tabela HTML com 6 colunas (Data, Descrição, Categoria, Conta, Valor, Ações) em substituição à lista de cards anterior, botão de editar inline por linha (bloqueado para transferências e parceladas), formulário com controle externo via `open`/`onOpenChange` e submissão PATCH, filtros redesenhados (selects arredondados, search com ícone), e responsividade mobile com bloco layout em viewports estreitos; novo arquivo `transaction-stats.tsx`, rewrite completo de `transaction-table.tsx`, suporte a edição em `transaction-form.tsx`, fix de sidebar em `app-shell.tsx`
