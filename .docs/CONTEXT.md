...
**Post-Phase 36 UI polish: botoes azuis e datepicker** — Concluida no codigo. O design system ganhou as variantes `choice` e `choice-active` no `Button`, reaproveitando o azul `#38BDF8` do botao de acao principal em seletores segmentados. O `DatePicker` em `src/components/ui/date-picker.tsx` usa `@base-ui/react` `Popover` com calendario customizado no visual blue/teal do sistema, suporta estado vazio/limpeza para campos opcionais e substitui inputs nativos de data nos fluxos de transacao, compra da wishlist, novo item desejado e recorrencia. As modais de nova transacao, registrar compra e nova regra recorrente agora usam o mesmo tom azul nos botoes de tipo/pagamento; os botoes `Comprar`, `Abrir produto` e `Ver compra parcelada` da wishlist ganharam efeito visual alinhado ao botao de criar transacao. O `Select` de conta da modal de nova transacao foi mantido sempre controlado com string para evitar o warning do Base UI ao alternar para transferencia. `npm run format`, `npm run lint` e `npm run build` passaram.
...
**Validacao visual/manual da nova UX de parcelamento e formularios** — rodar uma rodada manual focada em `/transactions`, `/credit-cards`, `/credit-card-purchases/[id]`, `/wishlist` e `/recurring` para revisar copy, estados vazios, datepicker, botoes azuis e clareza do fluxo de adiantamento no produto real.
